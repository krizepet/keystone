...
var EditForm = React.createClass({
            ...
            if (data.createdAt) {
                elements.createdAt = (
                    <div className="item-details-meta-item">
                        ...
                        <span
                            className="item-details-meta-info">{moment(data.createdAt).format('Do MMM YY h:mm:ssa')}</span>
                    </div>
                );
            }
        }
            ...
            if (data.createdBy) {
                ...
                elements.createdBy = (
                    <div className="item-details-meta-item">
                        ...
                        <span
                            className="item-details-meta-info">{data.createdBy.name.first} {data.createdBy.name.last}</span>
                    </div>
                );
            }
        }
            ...
            if (data.updatedAt && (!data.createdAt || data.createdAt !== data.updatedAt)) {
                elements.updatedAt = (
                    <div className="item-details-meta-item">
                        ...
                        <span
                            className="item-details-meta-info">{moment(data.updatedAt).format('Do MMM YY h:mm:ssa')}</span>
                    </div>
                );
            }
        }
            ...
            if (data.updatedBy && (!data.createdBy || data.createdBy.id !== data.updatedBy.id || elements.updatedAt)) {
                ...
                elements.updatedBy = (
                    <div className="item-details-meta-item">
                        <span className="item-details-meta-label">{label}</span>
                        <span
                            className="item-details-meta-info">{data.updatedBy.name.first} {data.updatedBy.name.last}</span>
                    </div>
                );
            }
        }
    ...
    renderToolbar: function () {
        ...
        if (!this.props.list.noedit) {
            toolbar.save = <button type="submit" className="btn btn-save save-buttonek">Save</button>;
            toolbar.saveAndRedirect =
                <button onClick={(e)=> {e.preventDefault()}} className="btn btn-save save-and-return">Save and
                    return</button>;

            if (this.props.preview) {
                toolbar.saveAndPreview =
                    <button onClick={(e)=> {e.preventDefault()}} className="btn btn-save save-and-preview">Save and
                        preview</button>;
            }
            // TODO: Confirm: Use React & Modal
            toolbar.reset = <a href={'/keystone/' + this.props.list.path + '/' + this.props.data.id}
                               className="btn btn-link btn-cancel"
                               data-confirm="Are you sure you want to reset your changes?">reset changes</a>;
        }

        if (!this.props.list.noedit && !this.props.list.nodelete) {
            // TODO: Confirm: Use React & Modal
            toolbar.del =
                <a href={'/keystone/' + this.props.list.path + '?delete=' + this.props.data.id + Keystone.csrf.query}
                   className="btn btn-link btn-cancel delete"
                   data-confirm={'Are you sure you want to delete this?' + this.props.list.singular.toLowerCase()}>delete {this.props.list.singular.toLowerCase()}</a>;
        }
